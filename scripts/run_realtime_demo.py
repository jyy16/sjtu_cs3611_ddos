#!/usr/bin/env python3
"""Python entrypoint for the realtime DDoS defense demo.

The realtime implementation already lives in scripts/run_realtime_demo.sh.
...
def main(argv: list[str] | None = None) -> int:
    ...
    shell_script = root_dir / "scripts" / "run_realtime_demo.sh"

    if not shell_script.is_file():
        print(f"[realtime-demo][error] missing shell script: {shell_script}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        command_text = f"cd {shlex.quote(bash_root)} && exec bash scripts/run_realtime_demo.sh"
        if args:
        ...
        print(
            f"[realtime-demo][error] bash executable not found: {bash_bin}. "
